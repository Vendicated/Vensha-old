...
namespace Vensha.Commands.Fun {
    public class HowHaram : ModuleBase<SocketCommandContext> {
        ...
        public async Task Run (Discord.IUser user) {
            await Result (user?.Username);
        }

        [Command ("howharam")]
        [Alias ("howhalal", "haram", "halal")]
        [Summary ("Find out how haram something is")]
        public async Task Run ([Remainder] string arg = null) {
            await Result (arg ?? Context.User.Username);
        }

        private async Task Result (string arg) {
